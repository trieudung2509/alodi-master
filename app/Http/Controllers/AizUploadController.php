...
use App\DataTransferObjects\UploadDto;
...
class AizUploadController extends Controller
{
    ...
    public function index(Request $request){
        ...
        if ((!isset($request->width) || trim($request->width) === '') == false) {
            $all_uploads = $all_uploads->where('width', '=', $request->width);
            $sort_width = $request->width;
        }
        if ((!isset($request->height) || trim($request->height) === '') == false) {
            $all_uploads = $all_uploads->where('height', '=', $request->height);
            $sort_height = $request->height;
        }
    ...
    public function upload(Request $request){
        ...
        if($request->hasFile('aiz_file')){
            ...
            if(isset($type[$extension])){
                ...
                return json_encode(new UploadDto(
                    $upload->width,
                    $upload->height,
                    URL::to('/public/').'/'.$path,
    ...
    public function get_uploaded_file_by_url(Request $request) {
        if ($request->has('url')) {
            $url = $request->url;
            $upload = Upload::where('file_name', '=', $url)->first();
            if ($upload == null)
            ...
            return json_encode(new UploadDto(
                $upload->width,
                $upload->height,
                URL::to('/public/').'/'.$upload->file_name,
                $upload->file_original_name
            ));
        }

        return response()->json([
            'message' => 'Url must be specified'], 400);
    }
    ...
    public function update(Request $request, $id) {
        ...
        if ($request->has('file_original_name')) {
            ...
            flash(translate('Uploaded file has been updated successfully'))->success();
        }
