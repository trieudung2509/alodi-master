            ...
            $(".aiz-text-editor").each(function (el) {
                ...
                buttons = !buttons
                    ? [
                          ...
                          ["style", ["style"]],
                          ["color", ["color"]],
                          ["insert", ["link", "picture", "video", "tableofcontent"]],
                ...
                $this.summernote({
                    ...
                    callbacks: {
                        onImageUpload: function(files) {
                            ...
                            $.ajax({
                                ...
                                success: function(result, status, xhr) {
                                    const uploadDto = JSON.parse(result);
                                    $this.summernote('insertImage', uploadDto.url, function (image) {
                                        image.attr('alt', uploadDto.alt);
                                    });
                                }
                            });
                        },
                        onImageLinkInsert: function(url) {
                            ...
                            let data = new FormData();
                            let fileName = url.split(AIZ.data.appUrl + "/public/")[1];
                            ...
                            $.ajax({
                                ...
                                type: "POST",
                                url: AIZ.data.appUrl + "/aiz-uploader/get_uploaded_file_by_url",
                                cache: false,
                                processData: false,
                                success: function (result, status, xhr) {
                                    const uploadDto = JSON.parse(result);
                        ...
                        onKeyup: function(e) {
                            ...
                            //Update value
                            $("#total-words").text(numWords);
                            $("#input-total-words").val(numWords);
                        ...
                        onPaste: function (e) {
                            if(format){
                                const eventData = ((e.originalEvent || e).clipboardData || window.clipboardData);
                                let bufferText = eventData.getData('text/html');
                                if (bufferText === '') {
                                    bufferText = eventData.getData('text');
                                }
                                bufferText = bufferText.replace(/<!--([^-]|-[^-]|--+[^->])*--+>/gm, ""); //remove html comments
                                e.preventDefault();
                                var div = $('<div />');
                                ...
                                div.find('*').removeClass('MsoNormal').removeAttr('face align').css({
                                    'font-family': '',
                                    'font-size': '',
                                    'margin': '',
                                    'line-height': '',
